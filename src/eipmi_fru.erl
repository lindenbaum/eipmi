...
do_read({ok, FruInfo}, SessionPid, FruId) ->
    ...
    decode(
        FruId,
        do_read(SessionPid, FruId, AreaSize, ?MAX_READ_COUNT div Div)
    ).
...
do_read(_SessionPid, _FruId, Size, _BlockSize, {Size, Acc}) ->
    Acc;
do_read(SessionPid, FruId, Size, BlockSize, {Offset, Acc}) when
...
decode_record_field_definition(
    Type,
    <<ManufaturerId:24/little, Rest/binary>>
...
%%------------------------------------------------------------------------------
decode_power_supply(
    <<OCapacity:16/little, PeakVA:16/little, InrushCurrent:8, InrushInterval:8,
        LowEndInputV1:16/little, HighEndInputV1:16/little,
        LowEndInputV2:16/little, HighEndInputV2:16/little,
        LowEndInputFrequency:8, HighEndInputFrequency:8, ACDropOutTolerance:8,
        0:3, FailSignal:1, HotSwap:1, Autoswitch:1, PowerFactor:1,
        PredictiveFailSupport:1, PeakW:16/little, CombinedWattageV1:4,
    ...
    [
        ...
        {predictive_fail_pin_supported,
            eipmi_util:get_bool(PredictiveFailSupport)},
...
%%------------------------------------------------------------------------------
decode_dc_output(
    <<Standby:1, ?EIPMI_RESERVED:3, OutputNumber:4,
        ...
        RippleAndNoise:16/little, MinimumCurrentDraw:16/little,
...
%%------------------------------------------------------------------------------
decode_dc_load(
    <<?EIPMI_RESERVED:4, OutputNumber:4, NominalVoltage:16/signed-little,
        MinimumVoltage:16/signed-little, MaximumVoltage:16/signed-little,
        RippleAndNoise:16/little, MinimumCurrentLoad:16/little,
...
decode_compatibility(
    <<ManufacturerID:24/little, EntityID:8, CompatibilityBase:8,
        ?EIPMI_RESERVED:1, CodeStart:7, CodeRanges/binary>>
) ->
...
decode_mtca_carrier_information_record(
    Acc,
    <<SiN:8, SiT:8, SlN:8, TiN:8, Y:16/little, X:16/little, Rest/binary>>
) ->
    decode_mtca_carrier_information_record(
        Acc ++
            [
                {slot,
                    eipmi_response:get_picmg_site_type(SiT) ++
                        [
                            {site_number, SiN},
                            {slot_number, SlN},
                            {tier_number, TiN},
                            {x, {X, eipmi_sensor:get_unit(32)}},
                            {y, {Y, eipmi_sensor:get_unit(32)}}
                        ]}
            ],
        Rest
    ).
...
get_amc_lanes(Lanes) ->
    A =
        case Lanes band 2#1000 bsr 3 of
            1 -> [3];
            _ -> []
        end,
    B =
        case Lanes band 2#100 bsr 2 of
            1 -> [2];
            _ -> []
        end,
    C =
        case Lanes band 2#10 bsr 1 of
            1 -> [1];
            _ -> []
        end,
    D =
        case Lanes band 2#1 of
            1 -> [0];
            _ -> []
        end,
    [{lanes, A ++ B ++ C ++ D}].
...
%% @private
%%------------------------------------------------------------------------------
get_link_type(16#02, Ext) ->
    [{link_type, {pci_express, Ext}}];
get_link_type(16#03, Ext) ->
    [{link_type, {pci_express_advanced_switching, Ext}}];
get_link_type(16#04, Ext) ->
    [{link_type, {pci_express_advanced_switching, Ext}}];
get_link_type(16#05, Ext) ->
    [{link_type, {ethernet, Ext}}];
get_link_type(16#06, Ext) ->
    [{link_type, {serial_rapid_io, Ext}}];
get_link_type(16#07, Ext) ->
    [{link_type, {storage, Ext}}];
...
decode_field(Name, Lang, <<3:2, Len:6, Data:Len/binary, Rest/binary>>) when
    Lang =:= ?ENGLISH orelse Lang =:= ?OLD_ENGLISH
->
