...
get_device_support(Support) ->
    A =
        case Support band 2#10000000 of
            2#10000000 -> [chassis];
            _ -> []
        end,
    B =
        case Support band 2#1000000 of
            2#1000000 -> [bridge];
            _ -> []
        end,
    C =
        case Support band 2#100000 of
            2#100000 -> [event_generator];
            _ -> []
        end,
    D =
        case Support band 2#10000 of
            2#10000 -> [event_receiver];
            _ -> []
        end,
    E =
        case Support band 2#1000 of
            2#1000 -> [fru_inventory];
            _ -> []
        end,
    F =
        case Support band 2#100 of
            2#100 -> [sel];
            _ -> []
        end,
    G =
        case Support band 2#10 of
            2#10 -> [sdr];
            _ -> []
        end,
    H =
        case Support band 2#1 of
            2#1 -> [sensor];
            _ -> []
        end,
    A ++ B ++ C ++ D ++ E ++ F ++ G ++ H.
...
%% @end
%%------------------------------------------------------------------------------
get_picmg_site_type(16#00) ->
    [{site_type, picmg_board}];
get_picmg_site_type(16#01) ->
    [{site_type, power_entry}];
get_picmg_site_type(16#02) ->
    [{site_type, shelf_fru_information}];
get_picmg_site_type(16#03) ->
    [{site_type, dedicated_shelf_management_controller}];
get_picmg_site_type(16#04) ->
    [{site_type, fan_tray}];
get_picmg_site_type(16#05) ->
    [{site_type, fan_filter_tray}];
get_picmg_site_type(16#06) ->
    [{site_type, alarm}];
get_picmg_site_type(16#07) ->
    [{site_type, amc}];
get_picmg_site_type(16#08) ->
    [{site_type, pmc}];
get_picmg_site_type(16#09) ->
    [{site_type, rear_transition_module}];
get_picmg_site_type(16#0a) ->
    [{site_type, mch}];
get_picmg_site_type(16#0b) ->
    [{site_type, power_module}];
...
%% @end
%%------------------------------------------------------------------------------
-spec decode_lan_configuration_parameters(
    pos_integer(),
    non_neg_integer(),
    binary()
...
decode_lan_configuration_parameters(
    ...
    _,
    <<?EIPMI_RESERVED:2, A:6, ?EIPMI_RESERVED:2, B:6, ?EIPMI_RESERVED:2, C:6,
        ?EIPMI_RESERVED:2, D:6, ?EIPMI_RESERVED:2, E:6, _/binary>>
) ->
    [
...
decode_lan_configuration_parameters(
    ...
    _,
    <<?EIPMI_RESERVED:4, Sel:4, Ack:1, ?EIPMI_RESERVED:4, Type:3, Timeout:8,
        ?EIPMI_RESERVED:5, Retries:3, _/binary>>
) ->
    [
        {set, Sel},
...
decode_lan_configuration_parameters(
    ...
    _,
    <<?EIPMI_RESERVED:4, Sel:4, 0:4, ?EIPMI_RESERVED:4, ?EIPMI_RESERVED:7, Gw:1,
        I1, I2, I3, I4, M1, M2, M3, M4, M5, M6, _/binary>>
) ->
    [
        {set, Sel},
        {gateway,
...
decode_sensor_event(
    ?GET_SENSOR_READING,
    <<Reading:1/binary, Events:1, Scanning:1, Available:1, ?EIPMI_RESERVED:5,
        States/binary>>
) ->
    [
...
decode_application(
    ?GET_DEVICE_ID,
    <<Id:8, _:1, ?EIPMI_RESERVED:3, Revision:4, Operation:1, Major:7, Minor:8,
        IPMIVersion:1/binary, Support:8, Manufacterer:24/little,
...
decode_application(
    ...
    <<Channel:8, E:1, ?EIPMI_RESERVED:1, A:6, ?EIPMI_RESERVED:2, K:1, P:1, U:1,
        L:3, ?EIPMI_RESERVED:6, Two:1, One:1, ?EIPMI_RESERVED:32>>
) ->
    Extended =
        case E of
            1 ->
...
decode_storage(
    Cmd,
    <<Version:1/binary, Entries:16/little, Free:16/little, Addition:32/little,
        Erase:32/little, Overflow:1, _:3, Operations:4>>
...
decode_storage(Cmd, <<Reservation:16/little>>) when
    Cmd =:= ?RESERVE_SEL orelse Cmd =:= ?RESERVE_SDR_REPOSITORY
->
    [{reservation_id, Reservation}];
decode_storage(Cmd, <<Next:16/little, Data/binary>>) when
    Cmd =:= ?GET_SEL_ENTRY orelse Cmd =:= ?GET_SDR
->
    [{next_record_id, Next}, {data, Data}];
...
%%------------------------------------------------------------------------------
decode_transport(
    ?GET_IP_UDP_RMCP_STATISTICS,
    <<IPRx:16/little, IPRxHdErr:16/little, IPRxAddrErr:16/little,
        IPRxFrag:16/little, IPTx:16/little, UDPRx:16/little, RMCPRx:16/little,
        UDPRxProxy:16/little, UDPDr:16/little>>
) ->
    [
...
decode_chassis(
    ...
    <<?EIPMI_RESERVED:4, L:1, D:1, F:1, I:1, Fru:8, Sdr:8, Sel:8, Sm:8,
        Rest/binary>>
) ->
    Caps =
        case Rest of
...
decode_chassis(
    ?GET_CHASSIS_STATUS,
    <<?EIPMI_RESERVED:1, Policy:2, ControlFault:1, Fault:1, Interlock:1,
        Overload:1, OnOff:1, LastPowerRsn:8, ?EIPMI_RESERVED:1, Identifies:1,
        Identifying:2, FanFault:1, DriveFault:1, PanelLockout:1, Intrusion:1,
        Rest/binary>>
) ->
    Front =
        case Rest of
            ...
            <<DisableStandby:1, DisableDiagInterrupt:1, DisableReset:1,
                DisablePowerOff:1, StandbyDisabled:1, DiagInterruptDisabled:1,
                ResetDisabled:1, PowerOffDisabled:1>> ->
                [
...
decode_chassis(
    ...
    <<?EIPMI_RESERVED:4, Cause:4, Channel:8>>
) ->
    [
...
decode_chassis(
    ...
    <<?EIPMI_RESERVED:5, PowerUp:1, LastState:1, PowerOff:1>>
) ->
    [
...
decode_picmg_(?GET_ADDRESS_INFO, [MCH, Addr, FruId, SiteN, SiteT, Carrier]) ->
    [{mch_site_number, MCH}, {ipmb_address, Addr}] ++
        ...
        case SiteT of
            16#ff -> [];
            _ -> get_picmg_site_type(SiteT)
        end ++
        case Carrier of
            0 -> [];
            _ -> [{carrier_number, Carrier}]
        end;
...
decode_open_session_rs(
    <<?EIPMI_RESERVED:4, P:4, ?EIPMI_RESERVED:8, Sq:32/little, Ss:32/little,
        0:8, ?EIPMI_RESERVED:16, 8:8, ?EIPMI_RESERVED:2, A:6,
        ?EIPMI_RESERVED:24, 1:8, ?EIPMI_RESERVED:16, 8:8, ?EIPMI_RESERVED:2,
        I:6, ?EIPMI_RESERVED:24, 2:8, ?EIPMI_RESERVED:16, 8:8,
        ?EIPMI_RESERVED:2, E:6, ?EIPMI_RESERVED:24>>
) ->
    {ok, [
        {privilege, decode_privilege(P)},
        {rq_session_id, Sq},
...
decode_rakp2(<<?EIPMI_RESERVED:16, Sq:32/little>>) ->
    {error, [{rq_session_id, Sq}]};
decode_rakp2(
    <<?EIPMI_RESERVED:16, Sq:32/little, Rs:16/binary, Guid:128/little,
        Auth/binary>>
) ->
    {ok, [
        {rq_session_id, Sq},
...
get_self_test_result(16#57, Bitfield) ->
    A =
        case Bitfield band 2#10000000 of
            2#10000000 -> [sel];
            _ -> []
        end,
    B =
        case Bitfield band 2#1000000 of
            2#1000000 -> [sdr];
            _ -> []
        end,
    C =
        case Bitfield band 2#100000 of
            2#100000 -> [fru];
            _ -> []
        end,
    D =
        case Bitfield band 2#10000 of
            2#10000 -> [ipmb_signal_lines];
            _ -> []
        end,
    E =
        case Bitfield band 2#1000 of
            2#1000 -> [sdr];
            _ -> []
        end,
    F =
        case Bitfield band 2#100 of
            2#100 -> [fru];
            _ -> []
        end,
    G =
        case Bitfield band 2#10 of
            2#10 -> [boot_firmware];
            _ -> []
        end,
    H =
        case Bitfield band 2#1 of
...
get_auth_types(AuthTypes) ->
    A =
        case AuthTypes band 2#100000 of
            2#100000 -> [oem];
            _ -> []
        end,
    B =
        case AuthTypes band 2#10000 of
            2#10000 -> [pwd];
            _ -> []
        end,
    C =
        case AuthTypes band 2#100 of
            2#100 -> [md5];
            _ -> []
        end,
    D =
        case AuthTypes band 2#10 of
            2#10 -> [md2];
            _ -> []
        end,
    E =
        case AuthTypes band 2#1 of
            2#1 -> [none];
            _ -> []
        end,
    A ++ B ++ C ++ D ++ E.
...
get_login_status(LoginStatus) ->
    A =
        case LoginStatus band 2#100 of
            2#100 -> [non_null];
            _ -> []
        end,
    B =
        case LoginStatus band 2#10 of
            2#10 -> [null];
            _ -> []
        end,
    C =
        case LoginStatus band 2#1 of
            2#1 -> [anonymous];
            _ -> []
        end,
    A ++ B ++ C.
...
get_operations(Operations) ->
    A =
        case Operations band 2#1000 of
            2#1000 -> [delete];
            _ -> []
        end,
    B =
        case Operations band 2#100 of
            2#100 -> [partial_add];
            _ -> []
        end,
    C =
        case Operations band 2#10 of
            2#10 -> [reserve];
            _ -> []
        end,
    D =
        case Operations band 2#1 of
            2#1 -> [get_allocation_info];
            _ -> []
        end,
    A ++ B ++ C ++ D.
