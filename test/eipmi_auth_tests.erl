...
hmac_md5_test() ->
    ...
    Key =
        <<16#37, 16#8d, 16#60, 16#63, 16#fe, 16#16, 16#05, 16#2c, 16#cc, 16#40,
            16#bf, 16#88, 16#92, 16#af, 16#28, 16#4a>>,
    Clear = <<"abcdefghijklmnopqrstuvwxyz">>,
    Hashed =
        <<16#04, 16#c0, 16#46, 16#ef, 16#75, 16#a1, 16#ff, 16#11, 16#db, 16#4d,
            16#f3, 16#b2, 16#39, 16#21, 16#20, 16#59>>,
...
hmac_sha1_test() ->
    ...
    Clear = <<"abcdefghijklmnopqrstuvwxyz">>,
    Hashed =
        <<16#82, 16#5b, 16#5b, 16#c1, 16#7b, 16#56, 16#f9, 16#d9, 16#27, 16#72,
            16#76, 16#07>>,
    ?assertEqual(Hashed, eipmi_auth:hash(HashType, Key, Clear)).
...
hmac_sha256_test() ->
    ...
    Clear = <<"abcdefghijklmnopqrstuvwxyz">>,
    Hashed =
        <<16#33, 16#ca, 16#58, 16#57, 16#8a, 16#fc, 16#86, 16#e6, 16#6d, 16#24,
            16#8d, 16#98, 16#0f, 16#6d, 16#8d, 16#06>>,
    ?assertEqual(Hashed, eipmi_auth:hash(HashType, Key, Clear)).
...
none_encryption_test() ->
    ...
    ?assertEqual(
        Unencrypted,
        eipmi_auth:encrypt(EncryptType, Key, Unencrypted)
    ),
    ?assertEqual(
        Unencrypted,
        eipmi_auth:decrypt(EncryptType, Key, Unencrypted)
    ).
